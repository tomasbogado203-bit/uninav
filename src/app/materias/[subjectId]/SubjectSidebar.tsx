  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="absolute -right-3 top-6 z-20 flex h-6 w-6 items-center justify-center rounded-full border border-slate-700 bg-slate-800 text-slate-300 shadow-md hover:bg-slate-700 hover:text-white transition-all text-xs cursor-pointer"
        title={collapsed ? 'Expandir barra lateral' : 'Colapsar barra lateral'}
      >
        {collapsed ? '▶' : '◀'}
      </button>

      {/* Header 1: Logo Marca UniNav */}
      ...
      {/* Sección 2: Navegación Global */}
      <div className="p-3 border-b border-slate-800/80 flex flex-col gap-1">
        {!collapsed && (
      ...
      {/* Sección 3: Header del Workspace de la Materia */}
      <div className="flex items-center justify-between border-b border-slate-800/80 p-4 bg-slate-900/60">
        {!collapsed ? (
          <div className="flex flex-col gap-0.5 overflow-hidden">
            ...
            <h2 className="text-sm font-bold text-white truncate" title={subjectName}>
              {subjectName}
            </h2>
          </div>
        ) : (
          <span className="mx-auto text-xs font-bold text-indigo-400">📖</span>
        )}
      </div>

      {/* Sección 4: Pestañas del Workspace */}
      <div className="flex-1 overflow-y-auto px-2 py-3 flex flex-col gap-1 scrollbar-none">
        {navItems.map((item) => {
          ...
          return (
            <div key={item.href} className="flex flex-col gap-0.5">
              ...
              {/* Submenú Gestor de Temas (Chats) */}
              {!collapsed && item.hasSubmenu && (
                <div className="ml-4 pl-3 border-l border-slate-800 flex flex-col gap-1 py-1 my-0.5">
                  <div className="flex items-center justify-between px-2 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                    <span>Temas</span>
                    <button
                      type="button"
                  ...
                  {/* Lista de Temas con opciones de Renombrar y Eliminar */}
                  {threads.map((thread) => (
                    <div
                      key={thread.id}
                      ...
                      {editingThreadId === thread.id ? (
                        <div className="flex items-center gap-1 w-full">
                          ...
                          <button
                            ...
                            className="text-indigo-400 hover:text-indigo-300 font-bold px-1"
                          >
                            ✓
                          </button>
                        </div>
                        ...
                        <>
                          ...
                          {/* Acciones de Edición (Renombrar / Eliminar) */}
      ...
      {/* Sección 5: Footer con Perfil de Usuario y Configuración */}
      <div className="border-t border-slate-800/80 p-3 bg-slate-950 flex flex-col gap-2">
        {/* Pastilla de Racha en Footer */}
        {!collapsed && (
          <div className="flex items-center justify-between bg-amber-500/10 border border-amber-500/30 rounded-xl px-2.5 py-1 text-amber-300 text-xs">
            <span className="flex items-center gap-1.5 font-bold">
              <IconFlame className="w-3.5 h-3.5 text-amber-400" />
              Racha: {streakDays} {streakDays === 1 ? 'día' : 'días'}
            </span>
            <span className="text-[10px] text-amber-400 font-mono">Activo</span>
          </div>
        )}
        ...
        {showConfigModal && !collapsed && (
          <div className="mt-2 rounded-xl border border-slate-800 bg-slate-900 p-2.5 flex flex-col gap-1 text-xs shadow-lg animate-in fade-in slide-in-from-bottom-2">
            <div className="px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-500">
              Ajustes de Perfil
            </div>
            <Link
              href="/onboarding"
              className="px-2 py-1.5 rounded-lg text-slate-300 hover:bg-slate-800 hover:text-white transition-colors flex items-center justify-between"
            >
            ...
            <button
              ...
              className="w-full text-left px-2 py-1.5 rounded-lg text-rose-400 hover:bg-rose-950/40 hover:text-rose-300 transition-colors flex items-center justify-between font-semibold cursor-pointer"
            >
