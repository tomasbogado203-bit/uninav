        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-300 max-w-xl">
              Espacio de estudio universitario. Selecciona tu materia para acceder al tutor RAG socrático, simulador de parciales, calendario y pizarra.
            </p>
          </div>
        ...
        <div className="lg:col-span-7 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {subjects?.map((sub) => (
              <div
                key={sub.id}
                className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm flex flex-col justify-between gap-3 hover:border-slate-300 transition-all"
              >
                <div className="flex items-center justify-between">
                ...
                {/* Botones de Atajo Directo a cada Pestaña */}
                <div className="grid grid-cols-2 gap-1.5 pt-2 border-t border-slate-100 text-[11px] font-semibold">
                  <Link
                    href={`/materias/${sub.id}/temas`}
                    className="rounded-lg bg-emerald-50 text-emerald-700 hover:bg-emerald-100 p-2 text-center transition-colors flex items-center justify-center gap-1.5"
                  >
                  ...
                  <Link
                    ...
                    className="rounded-lg bg-purple-50 text-purple-700 hover:bg-purple-100 p-2 text-center transition-colors flex items-center justify-center gap-1.5"
                  >
                    <IconQuiz className="w-3.5 h-3.5" />
                    Simulador
                  </Link>
                  <Link
                    href={`/materias/${sub.id}/calendario`}
                    className="rounded-lg bg-amber-50 text-amber-800 hover:bg-amber-100 p-2 text-center transition-colors flex items-center justify-center gap-1.5"
                  >
                  ...
                  <Link
                    ...
                    className="rounded-lg bg-indigo-50 text-indigo-700 hover:bg-indigo-100 p-2 text-center transition-colors flex items-center justify-center gap-1.5"
                  >
